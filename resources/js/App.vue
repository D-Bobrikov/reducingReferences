  ...
  <div class="col-5 container text-center">
    ...
    <table v-if="links.total" class="table">
      ...
      <tr v-for="link in links.data" :key="link.id">
        <td>
          <a
            v-bind:href="link.full_reference"
          >http://{{ link.short_reference }}</a
          >
        </td>
        <td>
          <button
    ...
    <div v-if="links.total > 10" class="d-flex justify-content-evenly">
      <p>
        <a @click="prevPageUrl" href="#" class="link-underline-dark">Назад</a>
      </p>
      <p>
        <a @click="nextPageUrl" href="#" class="link-underline-dark">Вперёд</a>
      </p>
    </div>
